...
["Command", "Event", "RegisterSlash"].filter(Boolean).forEach((h) => {
  require(`./handler/${h}`);
});
...
try {
  ...
  console.log(
    `     ┃ `.bold.brightGreen +
      `Loading Slash Commands`.bold.brightGreen +
      " ".repeat(
        -1 + stringlength - ` ┃ `.length - `Loading Slash Commands`.length,
      ) +
      "┃".bold.brightGreen,
  );
  console.log(
  ...
  readdirSync("./commands/").forEach((dir) => {
    ...
    for (let file of commands) {
      ...
      console.log(
        colors.red(`Slash : `) +
          colors.green(`${dir} : `) +
          colors.yellow(file + " - " + "File Was Loaded"),
      );
      if (pull.name) {
...
client.distube
  ...
  .on('error', (channel, e) => {
    let embed = new Discord.EmbedBuilder()
      ...
      .setDescription(`**Visit This Website To Resolve Error :- [ClickHere](https://grobot.store/errors.html).**`)
      .setColor(`${process.env.ec}`)
      .setFooter({
        text: `${client.user.username} - ${process.env.year} ©`,
        iconURL: process.env.iconurl
  ...
  .on('searchNoResult', (message, query) => {
    let embed = new Discord.EmbedBuilder()
      ...
      .setDescription(`**Error (429 : Server Busy Due To Too Many Requestes)** \n> If You Think This Is An Error Please Report Us By Us "$report" Command.`)
      .setColor(`${process.env.ec}`)
      .setFooter({
        text: `${client.user.username} - ${process.env.year} ©`,
        iconURL: process.env.iconurl
