  ...
  async run(client, interaction) {
    ...
    if (interaction.options.getSubcommand() === "set") {
      ...
      const checkchannel = db.fetch(
        `chatbot_${interaction.guild.id}`,
        channel.id,
      );
      if (!checkchannel) {
        db.set(`chatbot_${interaction.guild.id}`, channel.id);
        return await interaction.followUp({
          content: `> Chat Bot Was Now Bounded To ${channel}.`,
        });
      }
      if (checkchannel) {
        db.set(`chatbot_${interaction.guild.id}`, channel.id);
        return await interaction.followUp({
          content: `> Chat Bot Was Now Updated To ${channel}.`,
        });
      }
    }
    if (interaction.options.getSubcommand() === "delete") {
      ...
      if (!checkchannel) {
        return await interaction.followUp({
          content: `> Chat Bot Was Not Bounded To ${channel}.`,
        });
      }
      if (checkchannel) {
        db.delete(`chatbot_${interaction.guild.id}`, channel.id);
        return await interaction.followUp({
          content: `> Chat Bot Was Now Deleted In ${channel}.`,
        });
      }
    }
  }
