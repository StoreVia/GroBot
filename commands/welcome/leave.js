      ...
      data: new SlashCommandBuilder()
        ...
        .addSubcommandGroup((group) =>
          group
            ...
            .addSubcommand((subcommand) =>
              subcommand.setName(`edit`).setDescription(`Edit Leave Message`),
            ),
        )
        .addSubcommand((subcommand) =>
          subcommand
  ...
  async run(client, interaction) {
    ...
    if (subcommand === "edit") {
      ...
      const leavetextedit1 = new TextInputBuilder()
        ...
        .setLabel("Send's This Text When Some One Leave Server.")
        .setStyle(TextInputStyle.Paragraph);
      const leavetextedit0 = new ActionRowBuilder().addComponents(
        leavetextedit1,
      );
    ...
    if (subcommand === "delete") {
      ...
      if (checkchannel) {
        ...
        await interaction.reply({
          content: `> Leaving Message System Was Now Disabled In ${channel}`,
          ephemeral: true,
        });
      }
      if (!checkchannel) {
        await interaction.reply({
          content: `> ${channel} Was Not Bounded To Leave System.`,
          ephemeral: true,
        });
      }
    }
    ...
    if (subcommand === "guide") {
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `> **Note: **Use \`:emoji_name:\` To Use Emoji In Leave Message,Use \`<a:emoji_name:emoji_id>\` To Use Emoji Gif In Leave Message.`,
        )
