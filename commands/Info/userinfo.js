...
const formattor = new Intl.ListFormat(`en-GB`, {
  style: `narrow`,
  type: `conjunction`,
});
...
module.exports = class Userinfo extends Command {
  ...
  async run(client, interaction) {
    ...
    let flog = filteredFlags.length
      ? formattor.format(filteredFlags.map((flags) => flag[flags]))
      : "None";
    ...
    const userEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "**Avatar: **",
          value: `> [ClickHere](${UserOption.displayAvatarURL({
            size: 4096,
            dynamic: true,
            format: "png",
          })})`,
        ...
        },
        {
          ...
          value: `\`\`\`> ${moment(UserOption.createdAt).format(
            `DD-MM-YYYY`,
          )}\`\`\``,
          inline: true,
        },
        {
          name: "**Server Joined: **",
          value: `\`\`\`> ${moment(UserOption1.joinedAt).format(
            `DD-MM-YYYY`,
          )}\`\`\``,
          inline: true,
        },
