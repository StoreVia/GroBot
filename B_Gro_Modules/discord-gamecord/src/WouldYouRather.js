...
module.exports = class WouldYouRather extends events {
  constructor(options = {}) {
    ...
    if (!options.errMessage)
      options.errMessage = "Unable To Connect To Api. Try Again Later.";
    ...
    if (typeof options.buttons !== "object")
      throw new TypeError("INVALID_BUTTON: buttons option must be an object.");
  ...
  async getWyrQuestion() {
    return await fetch("https://api.aniket091.xyz/wyr")
      .then((res) => res.json())
      .then((res) => res?.data)
      .catch((e) => {
        return {};
  ...
  async gameOver(msg, result) {
    ...
    const prnt1 = Math.floor(
      (parseInt(this.data.option1_votes) /
        (parseInt(this.data.option1_votes) +
          parseInt(this.data.option2_votes))) *
        100,
    );
