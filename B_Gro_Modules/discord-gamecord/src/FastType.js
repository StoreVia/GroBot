...
module.exports = class FastType extends events {
  constructor(options = {}) {
    ...
    if (!options.winMessage)
      options.winMessage =
        "You won! You finished the type race in {time} seconds with word per minute of {wpm}.";
    ...
    if (typeof options.embed.description !== "string")
      throw new TypeError(
        "INVALID_DESCRIPTION: embed description must be a string.",
      );
    if (typeof options.sentence !== "string")
      throw new TypeError("INVALID_SENTENCE: sentence must be a string.");
  ...
  async startGame() {
    ...
    collector.on("collect", (m) => {
      ...
      this.wpm = Math.floor(
        m.content.trim().length / ((this.timeTaken / 60000) % 60) / 5,
      );
      return this.gameOver(
        msg,
        m.content?.toLowerCase().trim() === this.options.sentence.toLowerCase(),
      );
    });
  ...
  gameOver(msg, result) {
    const FasttypeGame = {
      ...
      timeTaken: Math.floor(this.timeTaken / 1000),
      wpm: this.wpm,
    };
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(this.options.embed.title)
      .setDescription(
        GameOverMessage.replace(
          "{time}",
          Math.floor((this.timeTaken / 1000) % 60),
        ).replace("{wpm}", this.wpm),
      )
      .setAuthor({
        name: this.message.author.tag,
        iconURL: this.message.author.displayAvatarURL({ dynamic: true }),
