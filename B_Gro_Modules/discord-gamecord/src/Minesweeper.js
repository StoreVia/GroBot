...
module.exports = class Minesweeper extends events {
  constructor(options = {}) {
    ...
    if (!options.embed.description)
      options.embed.description =
        "Click On The Buttons To Reveal The Block. BTW There Are Some Mines🫣 In Some Blocks Be Safe.";
    ...
    if (!options.winMessage)
      options.winMessage = "You Won Game Without Touching Any Mines.";
    if (!options.loseMessage)
      options.loseMessage = "You Have Touched Mine. Better Luck Next Time";
    ...
    if (typeof options.emojis.flag !== "string")
      throw new TypeError("INVALID_EMOJIS: flag emoji must be a string.");
    if (typeof options.emojis.mine !== "string")
      throw new TypeError("INVALID_EMOJIS: mine emoji must be a string.");
    if (typeof options.mines !== "number")
      throw new TypeError("INVALID_MINES: mines option must be a number.");
    ...
    if (options.mines < 1 || options.mines > 24)
      throw new RangeError(
        "INVALID_MINES: mines option must be between 1 and 24.",
      );
  ...
  handleButtons(msg) {
    ...
    collector.on("end", async (_, reason) => {
      if (reason === "user" || reason === "idle")
        return this.gameOver(msg, this.foundAllMines());
    });
  }
  ...
  gameOver(msg, result) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(this.options.embed.title)
      .setDescription(
        result ? this.options.winMessage : this.options.loseMessage,
      )
      .setAuthor({
        name: this.message.author.tag,
        iconURL: this.message.author.displayAvatarURL({ dynamic: true }),
    ...
    return msg.edit({
      embeds: [embed],
      components: disableButtons(this.getComponents(true, result)),
    });
  }
  ...
  getMinesAround(x, y) {
    ...
    for (let row = -1; row < 2; row++) {
      for (let col = -1; col < 2; col++) {
        ...
        if (block.x < 0 || block.x >= 5 || block.y < 0 || block.y >= 5)
          continue;
        if (row === 0 && col === 0) continue;

        if (this.gameBoard[block.y * this.length + block.x] === true)
          minesAround += 1;
      }
    }
    return minesAround;
  ...
  showFirstBlock() {
    ...
    this.gameBoard[rBlock.y * this.length + rBlock.x] = this.getMinesAround(
      rBlock.x,
      rBlock.y,
    );
  }
