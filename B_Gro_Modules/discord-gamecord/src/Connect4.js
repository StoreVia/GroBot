...
module.exports = class Connect4 extends approve {
  constructor(options = {}) {
    ...
    if (!options.winMessage)
      options.winMessage = "{emoji} | **{player}** won the Connect4 Game.";
    if (!options.tieMessage)
      options.tieMessage = "The Game tied! No one won the Game!";
    if (!options.timeoutMessage)
      options.timeoutMessage = "The Game went unfinished! No one won the Game!";
    ...
    if (!options.rejectMessage)
      options.rejectMessage =
        "The player denied your request for a round of **Connect4**.";
    ...
    if (typeof options.emojis.board !== "string")
      throw new TypeError("INVALID_EMOJIS: board emoji must be a string.");
    ...
    if (typeof options.buttonStyle !== "string")
      throw new TypeError(
        "INVALID_BUTTON_STYLE: button style must be a string.",
      );
    if (typeof options.turnMessage !== "string")
      throw new TypeError("INVALID_MESSAGE: Turn message must be a string.");
    if (typeof options.winMessage !== "string")
      throw new TypeError("INVALID_MESSAGE: Win message must be a string.");
    if (typeof options.tieMessage !== "string")
      throw new TypeError("INVALID_MESSAGE: Tie message must be a string.");
  ...
  async connect4Game(msg) {
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `${this.message.author.tag} vs ${this.opponent.tag}`,
      });
  ...
  async handleButtons(msg) {
    ...
    collector.on("collect", async (btn) => {
      ...
      if (block.y === 0) {
        if (column > 3)
          msg.components[1].components[column % 4].setDisabled(true);
        else msg.components[0].components[column].setDisabled(true);
      }

      if (this.foundCheck(block.x, block.y) || this.isBoardFull())
    ...
    collector.on("end", async (_, reason) => {
      if (reason === "idle" || reason === "user") {
        return this.gameOver(
          msg,
          reason === "idle" ? "timeout" : this.isBoardFull() ? "tie" : "win",
        );
      }
    });
  }
  ...
  async gameOver(msg, result) {
    ...
    if (result === "win")
      Connect4Game.winner = this.player1Turn
  ...
  getPlayerEmoji() {
    return this.player1Turn
      ? this.options.emojis.player1
      : this.options.emojis.player2;
  }
  ...
  isBoardFull() {
    for (let y = 0; y < 6; y++) {
      for (let x = 0; x < 7; x++) {
        if (this.gameBoard[y * 7 + x] === this.options.emojis.board)
          return false;
      }
    }
    return true;
