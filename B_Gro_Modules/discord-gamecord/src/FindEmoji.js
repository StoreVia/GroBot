...
module.exports = class FindEmoji extends events {
  constructor(options = {}) {
    ...
    if (!options.embed.description)
      options.embed.description = "Remember the emojis from the board below.";
    if (!options.embed.findDescription)
      options.embed.findDescription =
    ...
    if (!options.winMessage)
      options.winMessage = "You won! You selected the correct emoji. {emoji}";
    if (!options.loseMessage)
      options.loseMessage = "You lost! You selected the wrong emoji. {emoji}";
    ...
    if (typeof options.embed.findDescription !== "string")
      throw new TypeError(
        "INVALID_EMBED: embed findDescription must be a string.",
      );
    ...
    if (typeof options.hideEmojiTime !== "number")
      throw new TypeError(
        "INVALID_TIME: HideEmoji time option must be a number.",
      );
    ...
    if (!Array.isArray(options.emojis))
      throw new TypeError("INVALID_EMOJIS: emojis option must be an array.");
  ...
  async startGame() {
    ...
    setTimeout(async () => {
      embed.setDescription(
        this.options.embed.findDescription.replace("{emoji}", this.emoji),
      );
      await msg.edit({
        embeds: [embed],
  ...
  gameOver(msg, result) {
    const FindEmojiGame = {
      ...
      selectedEmoji: this.selected,
      correctEmoji: this.emoji,
    };
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(this.options.embed.title)
      .setDescription(
        this.options[resultMessage + "Message"].replace("{emoji}", this.emoji),
      )
      .setFooter({
        text: this.message.author.tag,
        iconURL: this.message.author.displayAvatarURL({ dynamic: true }),
