...
module.exports = class TicTacToe extends approve {
  constructor(options = {}) {
    ...
    if (typeof options.opponent !== "object")
      throw new TypeError(
        "INVALID_OPPONENT: opponent option must be an object.",
      );
    ...
    if (!options.turnMessage)
      options.turnMessage = "{emoji} | Its Turn Of Player **{player}**.";
    if (!options.winMessage)
      options.winMessage = "{emoji} | **{player}** Won The Game.";
    if (!options.tieMessage) options.tieMessage = "Game Tied.";
    if (!options.timeoutMessage)
      options.timeoutMessage = "The Game Was Unfinished!";
    if (!options.requestMessage)
      options.requestMessage = "{player} Has Invited You For Game(TicTacToe).";
    if (!options.rejectMessage)
      options.rejectMessage = "The Player Rejected To Play With You.";
    ...
    if (typeof options.embed.overTitle !== "string")
      throw new TypeError("INVALID_EMBED: over title must be a string.");
    if (typeof options.embed.color !== "string")
      throw new TypeError("INVALID_EMBED: embed color must be a string.");
    if (typeof options.emojis !== "object")
      throw new TypeError("INVALID_EMOJIS: emojis option must be an object.");
    ...
    if (typeof options.emojis.blankButton !== "string")
      throw new TypeError(
        "INVALID_EMOJIS: blankButton emoji must be a string.",
      );
    if (typeof options.xButtonStyle !== "string")
      throw new TypeError(
        "INVALID_BUTTON_STYLE: xbutton style must be a string.",
      );
    if (typeof options.oButtonStyle !== "string")
      throw new TypeError(
        "INVALID_BUTTON_STYLE: obutton style must be a string.",
      );
    if (typeof options.turnMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Turn message option must be a string.",
      );
    ...
    if (typeof options.tieMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Tie message option must be a string.",
      );
    ...
    if (typeof options.timeoutTime !== "number")
      throw new TypeError("INVALID_TIME: time option must be a number.");
    if (options.playerOnlyMessage !== false) {
      if (!options.playerOnlyMessage)
        options.playerOnlyMessage = "This Interaction Is Not For You.";
      if (typeof options.playerOnlyMessage !== "string")
        throw new TypeError(
          "INVALID_MESSAGE: playerOnly Message option must be a string.",
        );
    }
  ...
  async TicTacToeGame(msg) {
    const embed = new EmbedBuilder()
      ...
      .addFields({
        name: this.options.embed.statusTitle,
        value: this.getTurnMessage(),
      });
  ...
  handleButtons(msg) {
    ...
    collector.on("collect", async (btn) => {
      ...
      if (
        btn.user.id !== this.message.author.id &&
        btn.user.id !== this.opponent.id
      ) {
      ...
      if (
        btn.user.id !==
        (this.player1Turn ? this.message.author : this.opponent).id
      )
        return;
      ...
      if (
        this.hasWonGame(1) ||
        this.hasWonGame(2) ||
        !this.gameBoard.includes(0)
      )
        collector.stop();
      if (this.hasWonGame(1) || this.hasWonGame(2))
        return this.gameOver(msg, "win");
  ...
  async gameOver(msg, result) {
    ...
    if (result === "win")
      TicTacToeGame.winner = this.hasWonGame(1)
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`**${this.message.author.tag} vs ${this.opponent.tag}**`)
      .addFields({
        name: this.options.embed.overTitle,
        value: this.getTurnMessage(result + "Message"),
      });

    return await msg.edit({
      embeds: [embed],
      components: disableButtons(this.getComponents()),
    });
  }

  isGameOver() {
    if (this.hasWonGame(1) || this.hasWonGame(2) || !this.gameBoard.includes(0))
      return true;
    return false;
  }
  ...
  getPlayerEmoji() {
    return this.player1Turn
      ? this.options.emojis.xButton
      : this.options.emojis.oButton;
  }
  ...
  getButton(btn) {
    if (btn === 1)
      return {
        emoji: this.options.emojis.xButton,
        style: this.options.xButtonStyle,
      };
    else if (btn === 2)
      return {
        emoji: this.options.emojis.oButton,
        style: this.options.oButtonStyle,
      };
