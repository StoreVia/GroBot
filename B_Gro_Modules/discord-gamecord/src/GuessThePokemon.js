...
module.exports = class GuessThePokemon extends events {
  constructor(options = {}) {
    ...
    if (!options.winMessage)
      options.winMessage = "You guessed it right! It was a {pokemon}.";
    if (!options.loseMessage)
      options.loseMessage = "Better luck next time! It was a {pokemon}.";
    if (!options.errMessage)
      options.errMessage = "Unable to fetch pokemon data! Please try again.";
  ...
  async startGame() {
    ...
    const result = await fetch("https://api.aniket091.xyz/pokemon")
      .then((res) => res.json())
      .catch((e) => {
        return {};
      });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        name: "Types",
        value: this.pokemon.types.join(", ") ?? "No Data",
        inline: true,
      })
      .addFields({
        name: "Abilities",
    ...
    const attachment = new AttachmentBuilder(this.pokemon.questionImage, {
    ...
    collector.on("collect", (m) => {
      ...
      return this.gameOver(
        msg,
        m.content?.toLowerCase() === this.pokemon.name.toLowerCase(),
      );
    });
  ...
  async gameOver(msg, result) {
    ...
    if (!result)
      return msg.edit({
        content: this.options.loseMessage.replace(
          "{pokemon}",
          this.pokemon.name,
        ),
        embeds: [],
    ...
    const attachment = new AttachmentBuilder(this.pokemon.answerImage, {
      name: "answer-image.png",
    });
    return msg.edit({
      content: this.options.winMessage.replace("{pokemon}", this.pokemon.name),
      embeds: [embed],
      files: [attachment],
    });
  }
