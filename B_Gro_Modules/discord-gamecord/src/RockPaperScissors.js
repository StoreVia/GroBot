...
module.exports = class RPSGame extends approve {
  constructor(options = {}) {
    ...
    if (!options.embed.description)
      options.embed.description = "Use The Button Below To Make A Choice.";
    ...
    if (!options.winMessage)
      options.winMessage = "**{player}** Congratulations!, You Won The Game! ";
    if (!options.tieMessage) options.tieMessage = "Draw. No One Won The Game!";
    if (!options.timeoutMessage)
      options.timeoutMessage = "No One Won The Game!";
    if (!options.requestMessage)
      options.requestMessage =
        "{player} Has Invited You For Game(RockPaperScissors).";
    ...
    if (typeof options.buttons !== "object")
      throw new TypeError("INVALID_BUTTONS: buttons option must be an object.");
    if (typeof options.buttons.rock !== "string")
      throw new TypeError("INVALID_BUTTONS: rock button must be a string.");
    ...
    if (typeof options.emojis.rock !== "string")
      throw new TypeError("INVALID_EMOJIS: rock emoji must be a string.");
    if (typeof options.emojis.paper !== "string")
      throw new TypeError("INVALID_EMOJIS: paper emoji must be a string.");
    ...
    if (typeof options.pickMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Pick message option must be a string.",
      );
    ...
    if (typeof options.timeoutMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Timeout message option must be a string.",
      );
  ...
  async RPSGame(msg) {
    ...
    collector.on("collect", async (btn) => {
      ...
      if (btn.user.id === this.message.author.id && !this.playerPick) {
        ...
        btn.followUp({
          content: this.options.pickMessage.replace("{emoji}", this.playerPick),
          ephemeral: true,
        });
      } else if (!this.opponentPick) {
        this.opponentPick = choice[btn.customId.split("_")[1]];
        btn.followUp({
          content: this.options.pickMessage.replace(
            "{emoji}",
            this.opponentPick,
          ),
          ephemeral: true,
    ...
    collector.on("end", async (_, reason) => {
      if (reason === "idle" || reason === "user")
        return this.gameOver(msg, this.getResult());
    });
  }
  ...
  async gameOver(msg, result) {
    ...
    if (result === "win")
      RPSGame.winner = this.player1Won()
