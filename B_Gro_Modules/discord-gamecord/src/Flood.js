...
module.exports = class Flood extends events {
  constructor(options = {}) {
    ...
    if (!options.winMessage)
      options.winMessage = "You Won The Game In **{turns}** Turns.";
    if (!options.loseMessage)
      options.loseMessage =
        "You Have Lost Game, BTW You Took **{turns}** Turns.";
    if (options.emojis && Array.isArray(options.emojis))
    ...
    if (typeof options.difficulty !== "number")
      throw new TypeError("INVALID_LENGTH: length option must be a number.");
  ...
  async startGame() {
    ...
    collector.on("collect", async (btn) => {
      ...
      const update = await this.updateGame(
        squares[btn.customId.split("_")[1]],
        msg,
      );
  ...
  async updateGame(selected, msg) {
    ...
    while (queue.length > 0) {
      ...
      if (!block || visited.some((v) => v.x === block.x && v.y === block.y))
        continue;
      ...
      if (this.gameBoard[index] === firstBlock) {
        ...
        if (
          !visited.some((v) => v.x === down.x && v.y === down.y) &&
          down.y < this.length
        )
          queue.push(down);
        ...
        if (
          !visited.some((v) => v.x === left.x && v.y === left.y) &&
          left.x >= 0
        )
          queue.push(left);
    ...
    if (this.turns >= this.maxTurns && !gameOver)
      return void this.gameOver(msg, false);
