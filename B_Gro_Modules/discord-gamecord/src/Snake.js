...
module.exports = class SnakeGame extends events {
  constructor(options = {}) {
    ...
    if (typeof options.emojis !== "object")
      throw new TypeError("INVALID_EMOJI: emojis option must be an object.");
    ...
    if (typeof options.emojis.down !== "string")
      throw new TypeError("INVALID_EMOJI: down emoji must be a string.");
    if (typeof options.emojis.left !== "string")
      throw new TypeError("INVALID_EMOJI: left emoji must be a string.");
    if (typeof options.emojis.right !== "string")
      throw new TypeError("INVALID_EMOJI: right emoji must be a string.");
    ...
    if (typeof options.stopButton !== "string")
      throw new TypeError(
        "INVALID_STOPBUTTON: StopButton option must be a string.",
      );
    if (!Array.isArray(options.foods))
      throw new TypeError("INVALID_FOODS: foods option must be an array.");
  ...
  isSnake(pos) {
    return (
      this.snake.find((snake) => snake.x == pos.x && snake.y == pos.y) ?? false
    );
  }
  ...
  gameOver(msg) {
    ...
    this.emit("gameOver", {
      result: this.snakeLength >= HEIGHT * WIDTH ? "win" : "lose",
      ...SnakeGame,
    });
