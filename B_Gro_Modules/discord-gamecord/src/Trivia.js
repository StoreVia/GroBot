...
module.exports = class Trivia extends events {
  constructor(options = {}) {
    ...
    if (!options.embed.description)
      options.embed.description = "You have 60 seconds to guess the answer.";
    ...
    if (!options.winMessage)
      options.winMessage =
        "You Have Won The Game. The Correct Answer Is **{answer}**";
    if (!options.loseMessage)
      options.loseMessage =
        "You Have Lost The Game. BTW The The Correct Answer Is **{answer}**";
    if (!options.errMessage)
      options.errMessage = "Unable to fetch question data! Please try again.";
    ...
    if (typeof options.difficulty !== "string")
      throw new TypeError(
        "INVALID_DIFICULTY: Difficulty option must be a string.",
      );
    ...
    if (typeof options.errMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Error message option must be a string.",
      );
    ...
    if (!["multiple", "single"].includes(options.mode))
      throw new TypeError(
        "INVALID_MODE: Mode option must be multiple or single.",
      );
    if (!difficulties.includes(options.difficulty))
      throw new TypeError(
        "INVALID_DIFFICULTY: Difficulty option must be a easy, medium or hard.",
      );
    if (options.playerOnlyMessage !== false) {
      if (!options.playerOnlyMessage)
        options.playerOnlyMessage = "Only {player} can use these buttons.";
  ...
  async startGame() {
    ...
    if (!this.trivia.question)
      return this.sendMessage({ content: this.options.errMessage });
  ...
  getComponents(gameOver) {
    ...
    if (this.options.mode === "multiple") {
      if (gameOver && !this.selected)
        this.selected = this.trivia.options.indexOf(this.trivia.answer) + 1;
      const style = buttonStyle(
        this.selected ? "SECONDARY" : this.options.buttonStyle,
      );
      const btn1 = new ButtonBuilder()
        .setStyle(style)
        .setCustomId("trivia_1")
        .setLabel(this.trivia.options[0]);
      const btn2 = new ButtonBuilder()
        .setStyle(style)
        .setCustomId("trivia_2")
        .setLabel(this.trivia.options[1]);
      const btn3 = new ButtonBuilder()
        .setStyle(style)
        .setCustomId("trivia_3")
        .setLabel(this.trivia.options[2]);
      const btn4 = new ButtonBuilder()
        .setStyle(style)
        .setCustomId("trivia_4")
        .setLabel(this.trivia.options[3]);
      row.addComponents(btn1, btn2, btn3, btn4);

      if (this.selected) {
        if (this.trivia.answer !== this.trivia.options[this.selected - 1])
          row.components[this.selected - 1].setStyle(
            buttonStyle(this.options.falseButtonStyle),
          );
        else
          row.components[this.selected - 1].setStyle(
            buttonStyle(this.options.trueButtonStyle),
          );
      }
    } else {
      ...
      const btn1 = new ButtonBuilder()
        .setStyle(
          buttonStyle(
            this.selected ? "SECONDARY" : this.options.trueButtonStyle,
          ),
        )
      ...
      const btn2 = new ButtonBuilder()
        .setStyle(
          buttonStyle(
            this.selected ? "SECONDARY" : this.options.falseButtonStyle,
          ),
        )
      ...
      if (this.selected) {
        if (this.selected === "True")
          btn1.setStyle(
            buttonStyle(
              this.selected === this.trivia.answer ? "SUCCESS" : "DANGER",
            ),
          );
        else
          btn2.setStyle(
  ...
  async getTriviaQuestion() {
    ...
    if (questionMode === "multiple") {
      ...
      this.trivia.options = shuffleArray(result.incorrect_answers).map((e) =>
        decode(e),
      );
    }
  }
