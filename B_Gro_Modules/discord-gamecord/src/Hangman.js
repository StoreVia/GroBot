...
module.exports = class Hangman extends events {
  constructor(options = {}) {
    ...
    if (!options.winMessage)
      options.winMessage = "You Have Won The Game. The Word Was **{word}**";
    if (!options.loseMessage)
      options.loseMessage =
        "You Have Lost The Game.BTW The Word Was **{word}**.";
    ...
    if (typeof options.hangman !== "object")
      throw new TypeError("INVALID_HANGMAN: hangman option must be an object.");
    if (typeof options.hangman.hat !== "string")
      throw new TypeError("INVALID_HANGMAN: hangman hat must be a string.");
    if (typeof options.hangman.head !== "string")
      throw new TypeError("INVALID_HANGMAN: hangman head must be a string.");
    ...
    if (typeof options.theme !== "string")
      throw new TypeError("INVALID_THEME: theme option must be a string.");
  ...
  handleButtons(msg) {
    ...
    collector.on("end", (_, reason) => {
      if (reason === "idle" || reason === "user")
        return this.gameOver(msg, this.foundWord());
    });
  }
  ...
  gameOver(msg, result) {
    ...
    embed.addFields({
      name: "Game Over",
      value: GameOverMessage.replace("{word}", this.word),
    });
  ...
  getWordEmojis() {
    return this.word
      ...
      .split("")
      .map((l) =>
        this.guessed.includes(l) ? getAlphaEmoji(l) : l === " " ? "⬜" : "🔵",
      )
      .join(" ");
  }
  ...
  getComponents(page) {
    ...
    const pageBtn = new ButtonBuilder()
      ...
      .setCustomId(pageID);
    const letterY = new ButtonBuilder()
      .setStyle(buttonStyle("PRIMARY"))
      .setLabel("Y")
      .setCustomId("hangman_Y");
    const letterZ = new ButtonBuilder()
      .setStyle(buttonStyle("PRIMARY"))
      .setLabel("Z")
      .setCustomId("hangman_Z");
