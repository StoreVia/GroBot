...
module.exports = class Wordle extends events {
  ...
  async getBoardImage() {
    const guess = this.guessed.length
      ? "&guessed=" + this.guessed.join(",")
      : "";
    return await new AttachmentBuilder(
      "https://api.aniket091.xyz/wordle?word=" + this.word + guess,
      { name: "wordle.png" },
    );
  }
  ...
  async startGame() {
    ...
    if (!this.word)
      this.word =
        words["wordle"][Math.floor(Math.random() * words["wordle"].length)];
    ...
    const filter = (m) =>
      m.author.id === this.message.author.id && m.content.length === 5;
    ...
    collector.on("collect", async (m) => {
      ...
      if (this.word === guess || this.guessed.length > 5)
  ...
  async gameOver(msg) {
    ...
    const GameOverMessage = this.guessed.includes(this.word)
      ? this.options.winMessage
      : this.options.loseMessage;
    this.emit("gameOver", {
      result: this.guessed.includes(this.word) ? "win" : "lose",
      ...WordleGame,
    });
