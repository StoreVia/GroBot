...
module.exports = class MatchPairs extends events {
  constructor(options = {}) {
    ...
    if (!options.embed.description)
      options.embed.description =
        "Click On The Buttons And Match The Items With Their Respective Pairs.";
    ...
    if (options.emojis.length < 12)
      throw new RangeError(
        "INVALID_EMOJIS: Emojis option must contain at least 12 emojis.",
      );
  ...
  getPairEmoji(emoji) {
    ...
    for (let y = 0; y < this.length; y++) {
      for (let x = 0; x < this.length; x++) {
        ...
        if (this.emojis[index] === emoji)
          emojis.push({ x: x, y: y, id: index });
      }
    }
    return emojis;
  }
  ...
  getComponents() {
    ...
    for (let y = 0; y < this.length; y++) {
      ...
      for (let x = 0; x < this.length; x++) {
        ...
        btn.removeLabel = function () {
          this.data.label = null;
          return this;
        };
        btn.removeEmoji = function () {
  ...
  gameOver(msg, result) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(this.options.embed.title)
      .setDescription(
        GameOverMessage.replace("{tilesTurned}", this.tilesTurned),
      )
      .setAuthor({
        name: this.message.author.tag,
        iconURL: this.message.author.displayAvatarURL({ dynamic: true }),
    ...
    return msg.edit({
      embeds: [embed],
      components: disableButtons(this.components),
    });
  }
  ...
  async handleButtons(msg) {
    ...
    collector.on("collect", async (btn) => {
      ...
      if (!this.selected) {
        ...
        const selectedBtn =
          this.components[this.selected.y].components[this.selected.x];
        const matched =
          emoji === selectedEmoji || selectedEmoji === "🃏" || emoji === "🃏";

        if (selectedEmoji === "🃏" || emoji === "🃏") {
          const joker = emoji === "🃏" ? this.selected : { x: x, y: y, id: id };
          const pair = this.getPairEmoji(this.emojis[joker.id]).filter(
            (b) => b.id !== joker.id,
          )[0];
          const pairBtn = this.components[pair.y].components[pair.x];

          pairBtn
            .setEmoji(this.emojis[pair.id])
        ...
        emojiBtn
          .setEmoji(emoji)
          .setStyle(buttonStyle(matched ? "SUCCESS" : "DANGER"))
          .setDisabled(matched)
          .removeLabel();
        selectedBtn
