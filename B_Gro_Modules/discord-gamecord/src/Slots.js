...
module.exports = class Slots extends events {
  constructor(options = {}) {
    ...
    if (!Array.isArray(options.slots))
      throw new TypeError("INVALID_SLOTS: slots option must be an array.");
  ...
  getBoardContent(showResult) {
    let board = "```\n-------------------\n";
    board += `${this.wrap(this.slot1, false)}  :  ${this.wrap(
      this.slot2,
      false,
    )}  :  ${this.wrap(this.slot3, false)}\n\n`;
    board += `${this.slots[this.slot1]}  :  ${this.slots[this.slot2]}  :  ${
      this.slots[this.slot3]
    } <\n\n`;
    board += `${this.wrap(this.slot1, true)}  :  ${this.wrap(
      this.slot2,
      true,
    )}  :  ${this.wrap(this.slot3, true)}\n`;
    board += "-------------------\n";

    if (showResult)
      board += `| : :   ${this.hasWon() ? "WON " : "LOST"}   : : |`;
    return board + "```";
  }
  ...
  wrap(s, add) {
    if (add)
      return s + 1 > this.slots.length - 1 ? this.slots[0] : this.slots[s + 1];
