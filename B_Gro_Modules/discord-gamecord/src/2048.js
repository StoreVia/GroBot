...
module.exports = class TwoZeroFourEight extends events {
  constructor(options = {}) {
    ...
    if (typeof options.emojis.up !== "string")
      throw new TypeError("INVALID_EMOJIS: up emoji must be an string.");
    if (typeof options.emojis.down !== "string")
      throw new TypeError("INVALID_EMOJIS: down emoji must be an string.");
    if (typeof options.emojis.left !== "string")
      throw new TypeError("INVALID_EMOJIS: left emoji must be an string.");
    if (typeof options.emojis.right !== "string")
      throw new TypeError("INVALID_EMOJIS: right emoji must be an string.");
    ...
    if (options.playerOnlyMessage !== false) {
      ...
      if (typeof options.playerOnlyMessage !== "string")
        throw new TypeError(
          "INVALID_MESSAGE: playerOnlyMessage option must be a string.",
        );
    }
  ...
  async getBoardImage() {
    const url =
      "https://api.aniket091.xyz/2048?board=" +
      this.gameBoard.map((c) => chars[c]).join("");
  ...
  placeRandomTile() {
    ...
    do {
      tilePos = {
        ...
        y: parseInt(Math.random() * this.length),
      };
    } while (this.gameBoard[tilePos.y * this.length + tilePos.x] != 0);
    this.gameBoard[tilePos.y * this.length + tilePos.x] =
      Math.random() > 0.8 ? 2 : 1;
  }
  ...
  async handleButtons(msg) {
    ...
    collector.on("collect", async (btn) => {
      ...
      if (direction === "up" || direction === "down")
        moved = this.shiftVertical(direction);
      if (direction === "left" || direction === "right")
        moved = this.shiftHorizontal(direction);
  ...
  isInsideBlock(pos) {
    return (
      pos.x >= 0 && pos.y >= 0 && pos.x < this.length && pos.y < this.length
    );
  }
    ...
    while (!set) {
      ...
      if (
        ...
        this.score += Math.floor(
          Math.pow(this.gameBoard[moveTo.y * this.length + moveTo.x], 2),
        );
        this.gameBoard[pos.y * this.length + pos.x] = 0;
