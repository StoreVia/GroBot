...
module.exports = class FishyGame extends events {
  constructor(options = {}) {
    ...
    if (!options.player.balance && options.player.balance !== 0)
      options.player.balance = 50;
    ...
    if (!options.fishes.common)
      options.fishes.common = { emoji: "🐟", price: 10 };
    if (!options.fishes.uncommon)
      options.fishes.uncommon = { emoji: "🐠", price: 20 };
    ...
    if (!options.catchMessage)
      options.catchMessage =
        "You caught a {fish}. You paid {amount} for the fishing rod.";
    if (!options.sellMessage)
      options.sellMessage =
        "You sold {amount}x {fish} {type} items for a total of {price}.";
    if (!options.noBalanceMessage)
      options.noBalanceMessage =
    ...
    if (!options.invalidAmountMessage)
      options.invalidAmountMessage =
        "Amount must be between 0 and fish max amount.";
    if (!options.noItemMessage)
      options.noItemMessage =
        "You don't have any of this item in your inventory.";
    ...
    if (typeof options.player !== "object")
      throw new TypeError("INVALID_PLAYER: player option must be an object.");
    if (typeof options.player.id !== "string")
      throw new TypeError("INVALID_PLAYER: player id must be a string.");
    ...
    if (typeof options.catchMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Catch message option must be a string.",
      );
    if (typeof options.sellMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: Sell message option must be a string.",
      );
    if (typeof options.noBalanceMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: noBalance message option must be a string.",
      );
    if (typeof options.invalidTypeMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: InvalidType message option must be a string.",
      );
    if (typeof options.invalidAmountMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: InvalidAmount message option must be a string.",
      );
    if (typeof options.noItemMessage !== "string")
      throw new TypeError(
        "INVALID_MESSAGE: noItem message option must be a string.",
      );
  ...
  async sellFish(type, amount) {
    ...
    if (!this.player.fishes[type])
      return this.sendMessage({ content: this.options.noItemMessage });
  ...
  async fishyInventory() {
    const fishes = ["Common", "Uncommon", "Rare"]
      .map(
        (e) =>
          `**\u2000${this.fishes[e.toLowerCase()].emoji} ${e} Fish** — ${
            this.player.fishes[e.toLowerCase()] || 0
          }`,
      )
      .join("\n\n");
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        fishes +
          `\n\n\u2000**${this.fishes.junk.emoji} Junk** — ${
            this.player.fishes.junk || 0
          }`,
      )
      .setTimestamp();
