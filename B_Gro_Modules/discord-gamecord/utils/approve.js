...
module.exports = class Approve extends events {
  constructor(options = {}) {
    ...
    if (!options.requestMessage)
      options.requestMessage = "{opponent}, {player} Has Invited You For Game.";
    if (!options.rejectMessage)
      options.rejectMessage = "The Player Rejected To Play With You.";
    if (!options.reqTimeoutMessage)
      options.reqTimeoutMessage = `Game Canceled As Player Didn't Respond.`;
  ...
  async approve() {
    return new Promise(async (resolve) => {
      ...
      const msg = await this.sendMessage({
        content: `${this.opponent}`,
        embeds: [embed],
        components: [row],
      });
      const collector = msg.createMessageComponentCollector({
        time: this.options.reqTimeoutTime,
      });
      ...
      collector.on("end", async (_, reason) => {
        ...
        if (reason === "time")
          embed.setDescription(
            formatMessage(this.options, "reqTimeoutMessage"),
          );
        this.emit("gameOver", {
          result: reason,
