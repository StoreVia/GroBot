...
module.exports = class InteractionCreate extends Event {
  ...
  async run(interaction) {
    ...
    if (interaction.type === InteractionType.ApplicationCommand) {
      ...
      if (
        ...
        return await interaction.followUp({
          content: "> Slash Commands Can Only Be Used In Server/Guilds.",
        });
      }
      if (!command) {
        await interaction.deferReply();
        return (
          (await interaction.followUp({
            content: `> This Isn't Avilable For Now.`,
            ephemeral: true,
    ...
    if (interaction.customId === "myModalWelcomeNew") {
      await interaction
        .reply({
          content: `> Done✅. Welcome Channel Was Now Set.`,
          ephemeral: true,
        })
        .then(() => {
    ...
    if (interaction.customId === "myModalWelcomeOld") {
      await interaction
        .reply({
          content: `> Done✅. Welcome Channel Was Now Updated.`,
          ephemeral: true,
        })
        .then(() => {
          const text = interaction.fields.getTextInputValue("text");
    ...
    if (interaction.customId === "myModalWelcomeTextEdit") {
      await interaction
        .reply({
          content: `> Done✅. Welcome Channel Text Now Updated.`,
          ephemeral: true,
        })
        .then(() => {
    ...
    if (interaction.customId === "myModalDmUserNew") {
      await interaction
        .reply({
          content: `> Done✅. Welcome User Dm Was Now Set.`,
          ephemeral: true,
        })
        .then(() => {
    ...
    if (interaction.customId === "myModalDmUserTextEdit") {
      await interaction
        .reply({
          content: `> Done✅. User Dm Text Message Was Now Updated.`,
          ephemeral: true,
        })
        .then(() => {
    ...
    if (interaction.customId === "myModalLeaveNew") {
      await interaction
        .reply({
          content: `> Done✅. Leave Channel Was Now Set.`,
          ephemeral: true,
        })
        .then(() => {
          const text = interaction.fields.getTextInputValue("text");
    ...
    if (interaction.customId === "myModalLeaveOld") {
      await interaction
        .reply({
          content: `> Done✅. Leave Channel Was Now Updated.`,
          ephemeral: true,
        })
        .then(() => {
          const text = interaction.fields.getTextInputValue("text");
    ...
    if (interaction.customId === "myModalLeaveEditText") {
      await interaction
        .reply({
          content: `> Done✅. User Leave Text Was Now Updated.`,
          ephemeral: true,
        })
        .then(() => {
          const text1 = interaction.fields.getTextInputValue("text1");
    ...
    if (interaction.customId === "myUpdate") {
      await interaction
        .reply({
          content: `> Done✅. Update Text To Database.`,
          ephemeral: true,
        })
        .then(() => {
          const text = interaction.fields.getTextInputValue("text");
    ...
    if (interaction.customId === "ticketopen") {
      ...
      const channelcheck = interaction.member.guild.channels.cache.find(
        (channel) =>
          channel.name ===
          `${interaction.user.username.toLowerCase()}_${interaction.user.id}`,
      );
      ...
      if (channelcheck) {
        ...
        const channel1 = await interaction.guild.channels
          .create({
          ...
          .then(async (channel) => {
            await interaction.deferReply({ ephemeral: true });
            await interaction.followUp({
              content: `Done✅. Check Out ${channel}.`,
            });
            ...
            const embed = new Discord.EmbedBuilder()
              ...
              .setDescription(
                `Please Wait Our Staff Will Arrive Soon To Help You.`,
              )
            ...
            channel.send({
              content: `<@&${role}>, ${interaction.user} Created Ticket!`,
              embeds: [embed],
    ...
    if (interaction.customId === "closeticket") {
      ...
      let msg = await interaction.reply({
        content: `<@&${role}>, ${interaction.user} Has Requested For Closing Ticket Please Confirm Before Deleting.`,
        components: [row],
      });
        ...
        if (i.customId === "yes") {
          ...
          if (interaction.member.roles.cache.has(`${role}`)) {
            ...
            interaction.channel.messages.fetch().then(async (messages) => {
              ...
              var result = await paste.uploadContent(a, {
              ...
              const embed = new EmbedBuilder()
                ...
                .setDescription(
                  `To See Logs Of The Ticket Created By <@!${
                    interaction.channel.topic
                  }> [ClickHere](${getPasteUrl(result)})`,
                )
                .addFields(
                  {
                    name: `**CreatedBy: **`,
